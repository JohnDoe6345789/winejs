...
## Project Objectives
- Provide a lightweight WineJS prototype (`index.*`) that can load a Windows executable in the browser, surface its printable console output, visualize extracted strings, and fake enough GUI plumbing to render windows on an HTML canvas.
...
## Repository Layout
- `index.*` – unified browser runtime that extracts printable strings from an `.exe`, logs them through a hooked `WriteConsole`, and paints a faux GUI window when heuristics suggest a graphical intent.
...
2. Build a sample executable, for example:
   ...
   ```
3. Open `index.html` in a modern browser, select one of the compiled `.exe` files, and watch the runtime surface the logged strings or render the simulated window.
