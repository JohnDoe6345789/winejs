...
- **Bitwise stack-prologue coverage:** The decoder/CPU pair now understand common `AND`/`OR` forms (including sign-extended immediates) so standard Windows prologues that align `RSP` or twiddle flags stay inside the simulator instead of tripping “unsupported opcode” halts. This closes a frequent gap encountered when bringing over LearnWin32 samples that rely on stack alignment before touching Win32 APIs.
